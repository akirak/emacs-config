...
This function moves the point to the end of the new replaced region.
Also returns the point."
          ...
          (unless (zerop (apply #'call-process-region
            ...
            (error "pandoc failed: %s"
                   (with-temp-buffer
                     (insert-file-contents errfile)
                     (buffer-string))))
          ;; Remove CUSTOM_ID properties which are generated by pandoc.
          (let ((regexp (org-re-property "CUSTOM_ID")))
            (while (re-search-backward regexp begin t)
              (org-entry-delete nil "CUSTOM_ID")))
