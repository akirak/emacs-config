...
#+begin_src emacs-lisp
  (setup key-translation-map
    (:status opinionated
      ...
      (keymap-set key-translation-map "M-`" "<f1>")))
#+end_src
*** Window management
#+begin_src emacs-lisp
            ...
            (org-dog-facade-datetree-file
             (skeleton-insert
              `(> "#+title: " _
                  ...
                  n "#+REVERSE_DATETREE_DATE_FORMAT: %Y-%m-%d %A" n "#+REVERSE_DATETREE_MONTH_FORMAT: %Y-%m" n "#+REVERSE_DATETREE_YEAR_FORMAT: %Y" n n "* Backlog" n "* " (format-time-string "%Y") n "* Activities"
...
#+begin_src emacs-lisp
  (setup akirak-org-clock
    ...
    (with-eval-after-load 'org-keys
      (keymap-set org-mode-map "C-c C-x C-i" #'akirak-org-clock-in))
...
#+begin_src emacs-lisp
  (setup (:package casual)
    ...
    (:with-feature image-mode
      ...
      (:bind "C-," casual-image-tmenu))
    (:with-feature dired
      (:bind "C-," casual-dired-tmenu))
    (:with-feature calc
      (:bind "C-," casual-calc-tmenu)))
#+end_src
...
#+begin_src emacs-lisp
  (setup (:package gptel)
    ...
    (:with-feature org
      (:bind "C-c C-x =" #'gptel-org-set-topic))
...
| [[elysium][elysium]]    |      |
| [[gptel][gptel]]      |      |
| [[gptel-aibo][gptel-aibo]] |      |
| [[minuet][minuet]]     |      |
#+END:

For LLM integration, gptel is the generic frontend and backend.

There are packages for streaming workflows.
Most of them uses gptel as the backend.
*** Extra dependencies
