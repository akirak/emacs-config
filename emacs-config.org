...
- [[#post-init][Post init]]
- [[#integrating-into-nixoshome-manager][Integrating into NixOS/Home-Manager]]
- [[#notes][Notes]]
...
*** dictionary
:PROPERTIES:
:CREATED_TIME: [2026-06-21 Sun 10:33]
:END:

#+begin_src emacs-lisp
  (setup dictionary
    (:with-map help-map
      (:keymap "w" #'dictionary-search)))
#+end_src
*** dired                                                    :dired:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup dired
    (:set dired-recursive-copies 'always
          ...
          dired-filename-display-length 60
          dired-listing-switches "-alh --time-style=long-iso")
    (:mode-repl akirak-shell-new-other-window)

    (:keymap "C-c C-w" #'wdired-change-to-wdired-mode)

    (advice-add 'dired-internal-do-deletions :before #'akirak-dired-ad-before-deletions)
    (advice-add 'dired-show-file-type :override #'ignore)
    (advice-add 'akirak-dired-ad-after-rename-subdir
                :after #'akirak-dired-ad-after-rename-subdir)

    (:with-feature dired-aux
      (:also-load akirak-dired)
      (advice-add 'dired-rename-file :before #'akirak-dired-ad-before-deleting))

    (:status opinionated
      (:keymap "R" (defun akirak/dired-do-rename (&optional arg)
                     "Rename the file at point or move the selected files.

  This is a custom wrapper around `dired-do-rename'. With a single
  universal prefix, disable `dired-vc-rename-file' temporarily to allow
  moving the selected file(s) to outside of the current Git repository."
                     (interactive "P")
                     (if (equal arg '(4))
...
- [[https://www.jamescherti.com/emacs-dired-configuration/][Fixing Emacs Dired Defaults: Settings for Better File Management │ James Cherti]]
...
#+begin_src elisp
  (setup display-line-numbers
    (:hook-into prog-mode-hook markdown-mode-hook
                yaml-ts-mode-hook))
#+end_src
*** edebug
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup eww
    (:also-load shr-tag-pre-highlight)
    (:with-hook eww-after-render-hook
      (:hook eww-readable
             visual-line-mode))
    (:keymap "C-c C-n" #'outline-next-heading
...
#+begin_src emacs-lisp
  (setup files
    ...
    (:when-loaded
      (:set locate-dominating-stop-dir-regexp
            (rx-to-string `(or (regexp ,locate-dominating-stop-dir-regexp)
                               ;; Prevent scanning of directories with extensive
                               ;; entries.
                               (and bol "/nix/store/" (+ (any "-._" alnum)) "/" eol))))
    ...
    (advice-add 'dir-locals-collect-variables
                :filter-args
                (defun akirak/ad-dir-locals-collect-variables (args)
                  (cons (car args)
                        (cons (abbreviate-file-name (cadr args))
                              (cddr args)))))
    ...
    (:status personal
      (let* (;; (mix-format '((eval . (mix-format-on-save-mode t))))
             ...
             ;; settings for lsp-proxy.
             ;; (lsp '((eval . (akirak/lsp-proxy-enable))))
             ;; Use the same configuration for now.
             ;; (lsp-no-formatter lsp)
             ...
             (copilot '((eval . (when (require 'copilot nil t)
                                  (copilot-mode t)))))
             (editorconfig '((eval . (editorconfig-mode t))))
             (default  `((astro-ts-mode . (,@lsp-no-formatter))
                         ...
                         (just-mode . (,@lsp-no-formatter))
                         (kotlin-ts-mode . (,@lsp))
                         (nael-mode . (,@lsp))
                         (dune-mode . (,@lsp-no-formatter))
                         (neocaml-mode . (,@lsp-no-formatter))
                         (neocamli-mode . (,@lsp-no-formatter))
                         (python-mode . (,@lsp-no-formatter))
                         (nix-ts-mode . (,@lsp-no-formatter ,@nixfmt))
                         (reason-mode . (,@lsp-no-formatter))
                         (reason-ts-mode . (,@lsp-no-formatter))
                         (rust-ts-mode . (,@lsp))
                         ...
                         (prog-mode . (,@editorconfig))
                         (text-mode . (,@editorconfig)))))
        (dir-locals-set-class-variables
         'personal
         (append `((nix-ts-mode . (,@lsp-no-formatter ,@nixfmt))
        ...
        (dir-locals-set-class-variables
         ...
         (cons `(prog-mode . (,@copilot))
               default)))
      (dir-locals-set-directory-class "~/build/" 'personal)
      (dir-locals-set-directory-class "~/work2/learning/" 'default)
      (dir-locals-set-directory-class "~/work2/personal/" 'personal)
      (dir-locals-set-directory-class "~/work2/foss/" 'default)
      (dir-locals-set-directory-class "~/work2/projects/" 'default)
...
#+begin_src emacs-lisp
  (setup indent
    ...
    (defun akirak/one-space-in-region (start end)
      (interactive "r")
      (let ((initial-end end))
        (save-excursion
          (goto-char end)
          ...
          (goto-char start)
          (while (re-search-forward (rx (+ blank)) end t)
            (unless (save-match-data
                      (looking-back (rx bol (+ blank)) (line-beginning-position)))
              (replace-match " ")))
          (- initial-end (marker-position end)))))

    (defun akirak/indent-region (start end &optional column)
      (interactive "r\nP")
      (let ((deleted (akirak/one-space-in-region start end)))
        (indent-region start (- end deleted) column)))

    (:global-keymap "<remap> <indent-region>" #'akirak/indent-region))
#+end_src
*** indent.c                                            :formatting:
:PROPERTIES:
:CREATED_TIME: [2022-01-03 Mon 23:59]
...
*** mwheel
:PROPERTIES:
:CREATED_TIME: [2025-05-02 Fri 13:30]
:END:

#+begin_src emacs-lisp
  (setup mwheel
    (:set mouse-wheel-tilt-scroll t))
#+end_src
*** nxml-mode
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup process
    ;; Raise the limit for LSP servers
...
#+begin_src emacs-lisp
  (setup python
    ...
    (:with-mode python-ts-mode
      (:mode-repl akirak/euporie-project)))
#+end_src
*** re-builder                                              :regexp:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup replace
    (:set query-replace-show-preview t)
...
#+begin_src emacs-lisp
  (setup simple
    (:set suggest-key-bindings nil
          ...
          shell-command-default-error-buffer "*shell-command stderr*")

    (:with-mode (eat-mode compilation-mode)
      (:local-set buffer-undo-list t))

    (:global-keymap "<remap> <count-words-region>" #'akirak/count-words-dwim
                    "<remap> <delete-horizontal-space>" #'cycle-spacing
                    "<remap> <upcase-word>" #'upcase-dwim
                    "<remap> <downcase-word>" #'downcase-dwim
                    "<remap> <capitalize-word>" #'akirak/capitalize-dwim
                    "C-c v" #'visible-mode
                    "<f12> x l" #'list-processes)

    (defun akirak/capitalize-dwim (&optional arg)
      (interactive "P")
      (if (use-region-p)
        ...
        (cl-flet
            ((swap-at-point ()
               (let ((c (char-after (point))))
                 (delete-char 1)
                 (insert-char (upcase c)))))
          (pcase arg
            ((and '(4)
                  (guard (fboundp 'akirak/string-inflection))
                  (let `(,begin . ,end)
                    (bounds-of-thing-at-point 'symbol)))
             (akirak/string-inflection begin end))
            ((or '-
                 (and (pred numberp)
                      (guard (< arg 0))))
             (save-excursion
    ...
    (defun akirak/kill-word-dwim (&optional arg)
      (interactive "P")
      (require 'thingatpt)
      (cond
       ((equal arg '(4))
        ...
        (let ((eol (line-end-position)))
          (kill-region (point)
                       (or (re-search-forward (rx (+ blank)) eol t)
                           eol))))
       ((looking-at (rx (syntax open-parenthesis)))
        (pcase-exhaustive (funcall show-paren-data-function)
          (`(,open-start ,_ ,_ ,close-end . ,_)
           (kill-region open-start close-end))))
       ((looking-at (rx (syntax string-quote)))
        (pcase-exhaustive (thing-at-point-bounds-of-string-at-point)
          (`(,begin . ,end)
           (kill-region begin end))))
       (t
        (kill-word arg))))

    (defun akirak/zap-char (arg char &optional interactive)
      (interactive
       (let ((arg (prefix-numeric-value current-prefix-arg)))
         (list arg
               (read-char-from-minibuffer (if (and (numberp arg)
                                                   (< arg 0))
                                              "Zap backward char: "
                                            "Zap up to char: ")
      ...
      (if (and (numberp arg)
          ...
          (let ((limit (line-beginning-position))
                (case-fold-search (if (and interactive (char-uppercase-p char))
                                    ...
                                    case-fold-search))
                (end (point)))
            (kill-region (search-backward (char-to-string char))
                         end))
        (zap-up-to-char arg char interactive)))

    (:with-map prog-mode-map
      ;; For `text-mode-map', use `akirak-paren-kill-word'.
      (:keymap "<remap> <kill-word>" #'akirak/kill-word-dwim))
...
#+begin_src emacs-lisp
  (setup subword
    (defun akirak/enable-superword-mode ()
      (superword-mode 1))
    (add-hook 'text-mode-hook #'akirak/enable-superword-mode))
#+end_src
*** tab-bar
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup tab-bar
    ...
    (:with-map tab-prefix-map
      (:keymap "=" #'tab-bar-move-window-to-tab)
      (:keymap "R" #'akirak/tab-bar-set-tab-name-from-org-clock)
      (:keymap-unset "1"))

    (:with-mode tab-bar-history-mode
      (:hook-into after-init-hook)
      (:keymap "<f7>" #'tab-bar-history-back
               "S-<f7>" #'tab-bar-history-forward))

    (:when-loaded
      (dolist (i (number-sequence 1 8))
        (global-set-key (kbd (format "C-M-%s" i))
                        `(lambda ()
                           (interactive)
...
#+begin_src emacs-lisp
  (setup window
    ...
    (:global-keymap "<f8>" #'window-toggle-side-windows)
...
#+end_src
*** COMMENT winner                                             :history:windows:ARCHIVE:
:PROPERTIES:
:CREATED_TIME: [2022-01-03 Mon 23:59]
:END:
...
#+begin_src emacs-lisp
  (setup xdisp
    ...
    (:with-mode eat-mode
      ...
      (:local-set redisplay-skip-fontification-on-input t
...
#+begin_src emacs-lisp
  (defun akirak/enable-fonts (&optional value skip-check)
    (let ((families (font-family-list))
      ...
      (pcase-dolist (`(,faces ,family . ,plist) value)
               ...
               "C-x g" (defun akirak-oahu-goto-context (&optional view)
                         ...
                         (pcase (or view oahu-last-view)
                           ...
                           (`(OrgBookmark . ,bookmark)
                            (org-rooted-view bookmark)))))))
#+end_src
**** COMMENT oahu-memento                                                      :ARCHIVE:
:PROPERTIES:
...
*** ob-duckdb
:PROPERTIES:
:CREATED_TIME: [2025-05-05 Mon 11:48]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package ob-duckdb)
    (:org-babel-language t))
#+end_src

*** ob-gptel
:PROPERTIES:
:CREATED_TIME: [2026-03-20 Fri 12:35]
:END:
#+begin_quote
Org-babel backend for GPTel AI interactions
[[https://github.com/jwiegley/ob-gptel]]
#+end_quote

#+begin_src emacs-lisp
  (setup (:package ob-gptel)
    (:org-babel-language t))
#+end_src
*** ob-graphql
:PROPERTIES:
    ...
    (:with-mode org-mode
      ...
      (:status opinionated
        ...
        (:keymap akirak/mode-prefix-key (lookup-key org-mode-map (kbd "C-c C-x"))
                 ...
                 ;; Override the default binding to `org-feed-update-all'.
                 "C-c C-x g" #'akirak-org-gh-transient
                ...
                (unless (eq this-command 'org-capture-kill)
                  ...
                  (let ((headline (org-get-heading t t t t)))
                    (when (string-empty-p headline)
                      (if (fboundp 'akirak-org-ai-set-heading)
                          (akirak-org-ai-set-heading)
                        (org-edit-headline)))))))
...
#+begin_src emacs-lisp
  (setup ol
    ...
    (:status opinionated
      (:global-keymap "C-c l"
                      (defun akirak-org-store-link (&optional arg)
                        (interactive "P")
                        (if arg
                            (akirak-org-copy-remote-link)
...
#+begin_src emacs-lisp
  (setup (:package orgabilize)
    ;; currently fails to build, and actually I seldom use the functionality that
    ;; depends on this program.
...
#+begin_src emacs-lisp
  (setup (:package orgabilize-playwright)
    ;; The playwright-mcp package from nixpkgs
      ...
      (advice-add 'org-meta-return
                  ...
                  (defun akirak/org-autolist-ad-around-org-meta-return (orig &rest args)
                    (if (and (eq this-command 'org-return)
                      ...
                      (apply orig args))))))
#+end_src
*** COMMENT org-bb                                                  :@extra:Org:ARCHIVE:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package org-dog)
    ...
    (:with-map org-dog-file-mode-map
      (:keymap "<remap> <akirak-files-previous-file>" #'org-dog-previous-sibling-buffer
               "<remap> <akirak-files-next-file>" #'org-dog-next-sibling-buffer))
    ...
    (:set org-dog-context-path-patterns
          `(("^\\.github/" . "services/github.com")
            ...
            ("lakefile\\.toml\\'" . "programming/lean4")
    ...
    (:with-feature org-dog-context
      (:when-loaded
        (add-to-list 'org-dog-context-major-mode-aliases '(nael-mode . "lean4"))))
    ...
    (:status personal
      (add-hook 'org-dog-clock-in-success-hook
                #'akirak-org-git-add-properties-if-none))
        ...
        (:defer 1
                (defun akirak/org-dog-load-on-startup ()
                  ...
                  (let ((inhibit-message t))
                    ...
                    ;; Enable only after the Org files have been loaded.
                    (add-hook 'after-change-major-mode-hook
                              #'akirak-org-dog-use-major-mode-templates)
  ...
  (setup (:package octopus)
    ...
    (:when-loaded
      (transient-append-suffix #'octopus-find-node
        ...
        '("b" "Placeholder" org-rooted-find-or-create))
...
#+begin_src emacs-lisp
  (setup (:package org-epubinfo)
    (:nixpkgs customPackages.epubinfo))
#+end_src
*** org-hyprctl
:PROPERTIES:
...
#+end_src
*** COMMENT org-memento                                             :@extra:Org:ARCHIVE:
:PROPERTIES:
...
#+begin_quote
Boards for Org
[[https://github.com/akirak/org-rooted]]
#+end_quote

#+begin_src emacs-lisp
  (setup (:package org-rooted)
    (:set org-rooted-show-archived-entries-in-view t
          org-rooted-ignored-group-heading-regexp (rx bol (or "Backlog"
                                                              "Authors"
                                                              (repeat 4 digit))))
    (:with-mode org-rooted-view-mode
      (:hook org-rooted-view-fold-mode))

    (:when-loaded
      (require 'akirak-org-capture)
      (:set org-rooted-default-capture-template
            (concat "* %i\n"
                    akirak-org-capture-default-drawer
                    "%?")))
    (:global-keymap "M-s M-p" #'org-rooted-find-or-create))
#+end_src
...
#+end_src
*** COMMENT org-transclusion                                                   :ARCHIVE:
:PROPERTIES:
...
*** capnp-ts-mode
:PROPERTIES:
:CREATED_TIME: [2026-08-11 Tue 10:26]
:END:
#+begin_quote
Tree-sitter mode for Cap'n Proto
[[https://github.com/akirak/capnp-ts-mode]]
#+end_quote

#+begin_src emacs-lisp
  (setup (:package capnp-ts-mode)
    (:file-match "\\.capnp\\'")

    (defun akirak/capnp-insert-id ()
      "Insert a new ID into the buffer."
      (interactive)
      (call-process "capnp" nil (list t nil) nil "id")))
#+end_src
*** cypher-ts-mode
:PROPERTIES:
:CREATED_TIME: [2026-09-01 Tue 23:59]
:END:
#+begin_quote
Cypher editing mode
#+end_quote
...
*** haskell-ts-mode
:PROPERTIES:
:CREATED_TIME: [2026-01-12 Mon 18:27]
:END:
#+begin_quote
...
*** hcl-ts-mode
:PROPERTIES:
:CREATED_TIME: [2026-04-26 Sun 23:03]
:END:
#+begin_quote
tree-sitter support for HCL
#+end_quote

#+begin_src emacs-lisp
  (setup (:package hcl-ts-mode)
    (:file-match "\\.tf\\'"
                 "\\.prisma\\'"
                 "\\.hcl\\'")
    (:reformatters
...
#+begin_src emacs-lisp
  (setup json-ts-mode
    (:file-match "\\.jsonc?\\'" "\\.lock\\'" "*.webmanifest")
...
#+begin_src emacs-lisp
  (setup js-ts-mode
    ...
    (defun akirak/javascript-syntax-setup ()
      (setq-local c-block-comment-starter "/* "
                  c-block-comment-ender " */"
                  c-block-comment-start-regexp "/\\*[[:blank:]]*"
                  c-block-comment-end-regexp "[[:blank:]]*\\*/"))

    (:hook akirak/javascript-syntax-setup))
#+end_src
*** kotlin-ts-mode
:PROPERTIES:
:CREATED_TIME: [2025-08-09 Sat 23:59]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup lua-ts-mode
    (:file-match "\\.lua\\'"))
#+end_src
*** COMMENT markdown-ts-mode                                                   :ARCHIVE:
:PROPERTIES:
:CREATED_TIME: [2026-06-07 Sun 22:29]
:END:
...
[[http://github.com/bbatsov/neocaml]]
#+end_quote

#+begin_src emacs-lisp
  (setup (:package neocaml)
    (:file-match "\\.ml\\(?:\\.[^z-a]+\\)?\\'")
    (:with-feature neocaml-mlx
      (:file-match "\\.mlx\\'"))

    (:with-mode (neocaml-mode neocamli-mode neocaml-mlx-mode)
      (:mode-repl akirak/utop-project)
      (:org-src-lang "ocaml")
      (:mode-project-roots "dune-project")
      (:local-set treesit-font-lock-level 4)
      (:set neocaml-use-prettify-symbols nil)

      ;; Load abbreviations
      (:also-load akirak-ocaml)
      (:local-set local-abbrev-table akirak-ocaml-abbrev-table))
...
*** reason-ts-mode
:PROPERTIES:
:CREATED_TIME: [2026-08-03 Mon 08:48]
:END:
#+begin_quote
Tree-sitter mode for Reason ML
[[https://github.com/akirak/reason-ts-mode]]
#+end_quote

#+begin_src emacs-lisp
  (setup (:package reason-ts-mode)
    (:org-src-lang "reason")
    (:file-match "\\.re\\'"))
#+end_src
*** rust-ts-mode
...
#+begin_src emacs-lisp
  (setup typescript-ts-mode
    ...
    (:with-mode tsx-ts-mode
      ...
      (with-eval-after-load 'akirak-paren
        (:hook (defun akirak/tsx-setup-paren-match ()
                 (add-hook 'akirak-paren-match-hook
                           (defun akirak/tsx-match-empty-tag ()
                             (when-let* ((node (treesit-node-at (point)))
                                         (type (treesit-node-type node))
                                         (parent (when (member type '("<" "/>"))
                                                   (treesit-node-parent node))))
                               (when (equal (treesit-node-type parent)
...
*** typespec-ts-mode
:PROPERTIES:
:CREATED_TIME: [2026-07-28 Tue 23:59]
:END:
#+begin_quote
Major mode for TypeSpec (using tree-sitter)
[[https://github.com/pradyuman/typespec-ts-mode]]
#+end_quote

#+begin_src emacs-lisp
  (setup (:package typespec-ts-mode)
    (:file-match "\\.tsp\\'"))
#+end_src
*** yaml-ts-mode
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup yaml-ts-mode
    (:file-match "\\.ya?ml\\'")
    (:ffap
     (defun akirak/github-actions-ffap-handler (string)
       (when (string-match (rx (group (+ nonl) "/" (+ nonl))
                               "@" (group (+ (any "-" alnum))))
                           string)
...
*** akirak-ai-prompt
#+begin_src elisp
  (setup akirak-ai-prompt
    (:global-keymap "C-c a" #'akirak-ai-prompt))
#+end_src
*** akirak-avy
#+begin_src emacs-lisp
  (setup akirak-avy
    (:global-keymap "C-;" #'akirak-avy-insert-symbol
                    "C-:" #'akirak-avy-insert-word
                    "C-'" #'akirak-avy-paragraph-char-2
...
#+begin_src elisp
  (setup akirak-compile
    ;; dasel is required to parse TOML
    (:nixpkgs dasel)
...
*** akirak-consult-edit
#+begin_src elisp
  (setup akirak-consult-edit
    (keymap-set ctl-x-r-map "/" #'akirak-consult-edit))
#+end_src
...
#+begin_src elisp
  (setup akirak-denote
    (:global-keymap "M-s C-M-p" #'akirak-denote-search))
#+end_src
*** akirak-disk
#+begin_src elisp
  (setup akirak-disk
...
*** akirak-effect-ts
#+begin_src elisp
  (setup akirak-effect-ts
    (:keymap "C-, y" #'akirak-effect-ts-insert-yield
             "C-, e" #'akirak-effect-ts-insert-effect
             "C-, f" #'akirak-effect-ts-insert-function
             "C-, g" #'akirak-effect-ts-insert-gen)
    (:org-template "software/npm/effect.org"))
#+end_src
*** akirak-eldoc
:PROPERTIES:
...
#+begin_src elisp
  (setup akirak-files
    (:global-keymap "C-c [" #'akirak-files-previous-file
                    "C-c ]" #'akirak-files-next-file)
    (keymap-set help-map "!" #'akirak-executable-find))
#+end_src
*** akirak-git
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup akirak-git
    (:global-keymap "<f12> g r" #'akirak-git-list-remote-pages))
#+end_src
*** akirak-git-commit
#+begin_src emacs-lisp
...
#+begin_src elisp
  (setup akirak-github
    (:global-keymap "<f12> g i" #'akirak-github-browse-issues-web
                    "<f12> g p" #'akirak-github-browse-prs-web
                    "<f12> g l" #'akirak-github-workflow-run-transient))
#+end_src
...
*** akirak-language
#+begin_src elisp
  (setup akirak-language
    (:global-keymap "<f12> l" #'akirak-language-set-input-method))
#+end_src
*** akirak-lmstudio
#+begin_src elisp
  (setup akirak-lmstudio
    (:global-keymap "<f12> a l" #'akirak-lmstudio-model-transient))
#+end_src
*** akirak-log
#+begin_src elisp
  (setup akirak-log
...
#+begin_src emacs-lisp
  (setup akirak-org
    (:with-feature org
      (:hook akirak-org-truncate-property-mode)
      (:with-map org-mode-map
        (:keymap "<remap> <point-to-register>" #'akirak-org-point-to-register
                 "<remap> <org-cycle>" #'akirak-org-cycle-dwim)))

    (:when-loaded
      ;; Alternative to the default implementation provided as
      ;; `org-enforce-todo-dependencies' option.
      (add-hook 'org-blocker-hook #'akirak-org-block-done-by-descendants))
...
#+begin_src emacs-lisp
  (setup akirak-org-clock
    ...
    (add-hook 'akirak-org-clock-pre-exit-hook
              (defun akirak/org-capture-save-wconf ()
                (when (equal org-clock-marker
                             (buffer-local-value
                              'org-capture-clock-was-started
                              (current-buffer)))
                  (akirak-org-clock-save-wconf))))
...
#+begin_src elisp
  (setup akirak-paren
    ...
    (:global-keymap "C-c %" #'akirak-paren-goto-match)
    (:with-map text-mode-map
      (:keymap "<remap> <kill-word>" #'akirak-paren-kill-word))
...
#+begin_src emacs-lisp
  (setup akirak-passage
    ...
    (with-eval-after-load 'auth-source
      ...
      (add-hook 'auth-sources 'passage)))
#+end_src
*** akirak-process-compose
#+begin_src elisp
  (setup akirak-process-compose
    (:nixpkgs yq))
#+end_src
*** akirak-project
#+begin_src emacs-lisp
  (setup akirak-project
    ...
    (with-eval-after-load 'project
      (add-hook 'project-find-functions
                ;; This should precede `project-try-vc' entry.
                (defun akirak/project-find-eglot-root (dir)
                  "Return the project root specific to LSP (eglot)."
                  (when (bound-and-true-p eglot-lsp-context)
                    (or (vc-file-getprop dir 'eglot-vc)
                        (when-let* ((dir (akirak-project-mode-root-directory dir)))
                          (let ((res (list 'eglot dir)))
                            (and res (vc-file-setprop dir 'eglot-vc res))
                            res))))))
      (cl-defmethod project-root ((project (head eglot)))
        (cadr project)))
...
#+begin_src elisp
  (setup akirak-codex
    (:when-loaded
      (akirak-codex-watch-sessions))
    ;; mcp-proxy is used to communicate with non-stdio MCP servers
    (:nixpkgs "mcp-proxy"))
#+end_src
*** akirak-shr
#+begin_src elisp
...
#+begin_src elisp
  (setup akirak-systemctl
    (:global-keymap "<f12> s s" #'akirak-systemctl
  ...
  (setup akirak-treesit
    ...
    (add-hook 'after-change-major-mode-hook
              (defun akirak/enable-treesit-mode ()
                (when (or (string-suffix-p "-ts-mode" (symbol-name major-mode))
                          (memq major-mode '(neocaml-mode
                                             neocamli-mode)))
  ...
  (setup akirak-window
    (cl-macrolet
      ...
      (dolist (entry `((,(major-modes help-mode devdocs-mode eldoc-mode)
                       ...
                       ((or ,(major-modes vterm-mode)
                            ...
                            "^\\*popup-")
                        ,(reuse-mode-window
                          (vterm-mode nix-repl-mode "^\\*popup-")
    ...
    (:status opinionated
      ...
      (:global-keymap "C-2" #'akirak-window-split-vertically
                      ...
                      "S-<home>" #'akirak-window-display-buffer-starts
                      "S-<f8>" #'akirak-window-convert-to-side-window
...
#+begin_src elisp
  (setup akirak-xdg
    (:global-keymap "<f12> x x" #'akirak-xdg-run-desktop-app))
#+end_src
** Packages                                                :packages:
:PROPERTIES:
:SORTING_TYPE: a
...
*** COMMENT academic-phrases                                           :writing:ARCHIVE:
:PROPERTIES:
...
#+end_src
*** COMMENT acp                                                             :AI:ARCHIVE:
:PROPERTIES:
:CREATED_TIME: [2026-01-05 Mon 09:39]
:END:
#+begin_quote
...
This is a dependency for agent-shell, but it can also be used as an independent library for custom ACP integrations, so it's configured separately here.
...
(setup (:package affe))
#+end_src
*** COMMENT agent-shell                                                     :AI:ARCHIVE:
:PROPERTIES:
:CREATED_TIME: [2026-01-05 Mon 09:40]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package agent-shell)
    (advice-add 'agent-shell--display-buffer :override #'pop-to-buffer))
#+end_src
...
For keybindings, refer to the configuration for akirak-avy.
...
:END:
#+begin_quote
Open github/gitlab/bitbucket/stash/gist/phab/sourcehut/gitea page from Emacs
#+end_quote

#+begin_src emacs-lisp
  (setup (:package browse-at-remote)
    (:global-keymap "<f12> g t" #'akirak-browse-at-remote
                    "<f12> g g"
                    (defun akirak/browse-at-remote ()
                      (interactive)
                    ...
                    "<f12> g w"
                    (defun akirak/browse-at-remote-kill (&optional arg)
                      (interactive "P")
                      (require 'browse-at-remote)
                      (unless (vc-git-root default-directory)
                        (user-error "Not in a Git repository"))
    ...
    (:when-loaded
      (:set consult-buffer-sources `(consult-source-project-buffer
            ...
            consult-bookmark-narrow '((?f "File or directory" bookmark-default-handler)
                                      ...
                                      (?v "Org Placeholder" org-rooted-view))
...
*** consult-eglot
:PROPERTIES:
:CREATED_TIME: [2026-03-31 Tue 23:59]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package consult-eglot)
    (add-hook 'eglot-managed-mode-hook
              (defun akirak/setup-consult-eglot ()
                (when (eglot-server-capable :workspaceSymbolProvider)
                  (keymap-local-set "<remap> <consult-imenu-multi>"
...
#+begin_src emacs-lisp
  (setup (:package d2-mode)
    ...
    (:nixpkgs d2 customPackages.d2-format)
    (:file-match "\\.d2\\'")
...
*** dag-draw                                                           :drawing:
:PROPERTIES:
:CREATED_TIME: [2025-11-12 Wed 20:49]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package dashboard)
    (:set dashboard-center-content t
          ...
          dashboard-item-shortcuts '((process-buffers . "b")
                                     (recents . "r")
                                     (bookmarks . "m")
                                     ;; Don't use p
                                     (projects . "j")
                                     ...
                                     (agenda . "a")
                                     (registers . "e"))
          dashboard-items '((process-buffers . 10)
                            (custom-agenda . 10)
                            (org-clock . 7)
                            (projects . 15)
                            (bookmarks . 10)
    ...
    (:when-loaded
      ...
      (add-to-list 'dashboard-item-generators
                   (cons 'org-clock 'akirak-dashboard-org-clock-insert))
      (add-to-list 'dashboard-item-generators
                   (cons 'custom-agenda 'akirak-dashboard-insert-agenda))
      (add-to-list 'dashboard-item-generators
                   (cons 'process-buffers 'akirak-dashboard-insert-process-buffers)))
...
*** denote
:PROPERTIES:
:CREATED_TIME: [2026-09-04 Fri 23:59]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package denote)
    (:set denote-org-store-link-to-heading 'link
          ;; Each denote entry must contain some front matter. Otherwise, denote
          ;; writes it.
          ;; denote-org-front-matter "#+denote_identifier: %4$s\n"
          denote-org-front-matter "")
    (add-hook 'denote-after-new-note-hook #'akirak-denote-visit-hook)

    (:when-loaded
      (let ((plist (cdr (assq 'org denote-file-types))))
        ;; (plist-put plist :identifier-key-regexp "^#\\+denote_identifier\\s-*:")
        (plist-put plist :title-key-regexp "^\\* "))

      (advice-add 'denote-retrieve-front-matter-title-value
                  :around
                  (defun akirak/ad-around-denote-title-value (orig file type)
                    (if (eq type 'org)
                        (org-with-base-buffer (get-file-buffer file)
                          (require 'akirak-denote)
                          (akirak-denote-org-title))
                      (apply orig file type))))))
#+end_src
*** devdocs                                              :reference:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package diff-hl)
    ...
    ;; Disable the dired integration as managing the background process is annoying.
    ;; (add-hook 'dired-mode-hook #'diff-hl-dired-mode)

    ;; `diff-hl-command-prefix' is ineffective since I've patched the source.
    (:set diff-hl-show-staged-changes t
          ;; https://github.com/magit/magit/discussions/5167
          diff-hl-update-async nil)

    ;; Missing autoload cookies
    (autoload 'diff-hl-next-hunk "diff-hl" nil 'interactive)
    (autoload 'diff-hl-previous-hunk "diff-hl" nil 'interactive)
    ...
    (:when-loaded
      ;; `diff-hl-command-map' is no longer available since I've patched the
      ;; source.
  ...
  (setup eglot
    ...
    (defun akirak/eglot-ensure ()
      (if (and (buffer-file-name)
               (not (bound-and-true-p magit-buffer-revision))
               ;; With unmerged changes, a LSP server may exit abnormally,
               ;; so don't run the server on such files.
    ...
    (defun akirak/eglot-rename-dwim ()
      (interactive)
      (or (akirak-treesit-maybe-rename-tag)
          (call-interactively #'eglot-rename)))

    (when (executable-find "rg")
      (cl-defmethod project-files ((project (head eglot)) &optional dirs)
        (mapcan
         (lambda (dir)
    ...
    (:with-feature typescript-ts-mode
      (:with-map tsx-ts-mode-map
        (:keymap "<remap> <eglot-rename>" #'akirak/eglot-rename-dwim)))
    ...
    (:when-loaded
      ...
      ;; Disable deep analysis of the source code
      ...
      ;; Never use folding provided by LSP
      ...
      (add-to-list 'eglot-server-programs
                   '((neocamli-mode :language-id "ocamli") . ("ocamllsp")))
      (add-to-list 'eglot-server-programs
                   '(lean4-mode . ("lake" "serve")))
      (add-to-list 'eglot-server-programs
                   '(coq-mode . ("coq-lsp")))
      ...
      (let ((java-debug-plugin (locate-user-emacs-file "java-debug-plugin.jar")))
        (when (file-exists-p java-debug-plugin)
          ;; Alternatively, set the variable eglot-workspace-configuration in the
          ;; file .dir-locals.el in a project’s root directory, to have JDTLS load
          ;; the plugin for that project.
          ;; https://github.com/svaante/dape?tab=readme-ov-file#java---jdtls-with-java-debug-server-plugin
          (add-to-list 'eglot-server-programs
                       `(java-ts-mode
                         . ("jdtls"
                            :initializationOptions
                            (:bundles [,java-debug-plugin]))))))
...
- [[https://www.jamescherti.com/emacs-eglot-performance/][Configuring Emacs Eglot for Optimal Performance │ James Cherti]]
**** ocaml-eglot
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package eglot-booster)
    ...
    (:set eglot-booster-no-remote-boost t
          ;; Needed to prevent conversion of mathematical symbols in Lean 4
  ...
  (setup (:package epkg-marginalia)
    (with-eval-after-load 'marginalia
      (cl-pushnew 'epkg-marginalia-annotate-package
                  (alist-get 'package marginalia-annotators))))
#+end_src
*** erlang                                                         :major__mode:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package flymake-sqlfluff)
    ...
    ;; Add to .dir-locals.el of individual projects instead.
    ;;
...
#+end_src
*** flymake-vale                                                :writing:@extra:
:PROPERTIES:
:CREATED_TIME: [2025-07-13 Sun 13:55]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package flymake-vale)
    (:nixpkgs vale)

    (:with-hook (org-mode-hook markdown-mode-hook)
      (:hook flymake-vale-load)))
#+end_src
*** flyover
:PROPERTIES:
:CREATED_TIME: [2025-12-06 Sat 23:59]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package flyover)
    (:set flyover-checkers '(flymake)
          ...
          flyover-background-lightness 90
...
#+end_src
*** COMMENT futur                                                              :ARCHIVE:
:PROPERTIES:
:CREATED_TIME: [2026-02-14 Sat 20:34]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup git-commit
    (:keymap "M-<return>" #'akirak/git-commit-auto-insert)

    (defcustom akirak/git-commit-auto-insert-hook
      '(akirak-git-commit-static-message
        akirak-org-clock-commit-title)
      "Hook that generates and inserts a commit message (or part of it).

  Each function in this hook is called inside a buffer for editing the
  commit message and must return non-nil for successful insertion."
      :type 'hook)

    (defun akirak/git-commit-auto-insert (&optional arg)
      (interactive "P")
      (if arg
          (progn
            (require 'akirak-ai-prompt)
            (akirak-ai-prompt-generate-git-commit-message))
        (unless (run-hook-with-args-until-success 'akirak/git-commit-auto-insert-hook)
          (let ((this-command 'auto-insert))
            (auto-insert)))))

    (:auto-insert
     ("\\.git/COMMIT_EDITMSG\\'"
      . (> _ (when-let* ((marker (and (featurep 'org-clock)
                                      (org-clocking-p)
                                      (or org-clock-hd-marker
                                          org-clock-marker)))
                         (heading (org-entry-get marker "ITEM")))
               (with-temp-buffer
                 (insert (akirak-pandoc-convert-string heading
                           :from "org" :to "gfm"))
                 (goto-char (point-min))
                 (when (looking-at (rx (+ (any word punct)) ": "))
...
#+begin_src emacs-lisp
  (setup (:package github-linguist)
    (:nixpkgs "customPackages.github-linguist")
...
#+begin_src emacs-lisp
  (setup (:package gptel)
    ...
    (:with-feature gptel-transient
      (:also-load gptel-integrations))

    (with-eval-after-load 'embark
      (keymap-set embark-region-map "!" #'gptel-rewrite-menu))

    (defun akirak/gptel-insert-model-id (&optional arg)
      (interactive "P")
      (require 'gptel)
      (let* ((provider (if arg
                           (completing-read "Provider: " gptel--known-backends
                                            nil t)
                         "OpenRouter"))
             (models (gptel-backend-models (gptel-get-backend provider)))
             (result (completing-read-multiple "Model: " models
                                               nil nil nil nil
                                               (symbol-name gptel-model)))
             (text (if (listp result)
                       (string-join result "\n")
                     result)))
        (cl-case (derived-mode-p 'eat-mode)
          (eat-mode
           (eat-term-send-string-as-yank eat-terminal text))
          (otherwise
           (insert text)))))
    ...
    (:when-loaded
      ...
      (:set gptel-backend (gptel-make-openai-oauth "OpenAI-sub")
      ...
      ;;         :stream t
      ;;         :models (akirak-ai-model-prefixed-list "openai/")))
      ...
      (gptel-make-openai "OpenCode Zen"
        ...
        :endpoint "/zen/v1"
        :key (lambda ()
               (gptel-api-key-from-auth-source "opencode.ai"))
        :models '(glm-5.2
                  kimi-k2.7-code
                  deepseek-v4-pro
                  deepseek-v4-flash)
...
- [X] OpenAI via subscription (default, requires ChatGPT Plus or higher)
- [X] OpenCode Go/Zen
- [ ] Locally-hosted llama-swap: Not part of this repository
...
#+end_src
*** COMMENT hcl-mode                                                 :Terraform:ARCHIVE:
:PROPERTIES:
:CREATED_TIME: [2022-03-20 Sun 23:59]
:END:
#+begin_quote
...
*** imenu-list
:PROPERTIES:
:CREATED_TIME: [2025-08-15 Fri 23:59]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package imenu-list)
    (:set imenu-list-auto-resize nil
          ;; Just manually update the list
...
*** kubernetes
:PROPERTIES:
:CREATED_TIME: [2026-08-28 Fri 23:59]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package lean4-mode)
    (:nixpkgs lean4)
    (:file-match "\\.lean\\'")
    (:set lean4-mode-hook '(lean4-input-set flymake-mode)))
#+end_src
...
#+begin_src emacs-lisp
  (setup (:package link-hint)
    (:global-keymap "M-g f" #'link-hint-open-link
                    ;; Override `xref-find-apropos'.
                    "C-M-." #'akirak/link-hint-embark-act)
    ...
    (defun akirak/link-hint-embark-act ()
      "Use avy to call 'embark-act' on a link.

  This is originally implemented by oldjawbone and posted at
  <https://www.reddit.com/r/emacs/comments/uek8mt/comment/i6pfz8y/>.

  To enable this action, you alslo have to define :embark-act action on
  each link-hint type."
      (interactive)
    ...
    (:when-loaded
      ;; Based on https://www.reddit.com/r/emacs/comments/uek8mt/comment/i6pfz8y/
      (mapc (lambda (symbol)
              (put symbol :embark-act #'(lambda () (call-interactively #'embark-act))))
            link-hint-types)
...
#+end_src
*** COMMENT lsp-proxy                                                          :ARCHIVE:
:PROPERTIES:
:CREATED_TIME: [2026-04-18 Sat 09:41]
:END:
#+begin_quote
LSP Proxy client for Emacs
#+end_quote
...
#+begin_src emacs-lisp
  (setup (:package lsp-proxy)
    ...
    (:keymap akirak/mode-prefix-key #'akirak/lsp-proxy-prefix-map)

    (defun akirak/lsp-proxy-enable ()
      (lsp-proxy-mode 1))

    (defun akirak/lsp-proxy-rename-dwim ()
      (interactive)
      (or (akirak-treesit-maybe-rename-tag)
    ...
    (:with-feature typescript-ts-mode
      (:with-map tsx-ts-mode-map
        (:keymap "<remap> <lsp-proxy-rename>" #'akirak/lsp-proxy-rename-dwim)))

    (:when-loaded
      (define-prefix-command 'akirak/lsp-proxy-prefix-map)
      ;; Avoid the global /tmp directory
      (:set lsp-proxy-log-file-directory
            (file-name-concat (xdg-runtime-dir) "emacs-lsp-proxy"))
      (:with-map akirak/lsp-proxy-prefix-map
        (:keymap "a" #'lsp-proxy-execute-code-action
                 "d" #'lsp-proxy-describe-thing-at-point
                 "f" #'lsp-proxy-format-buffer
                 "R" #'lsp-proxy-restart
                 "r" #'lsp-proxy-rename
                 "c" #'lsp-proxy-inline-completion-mode))))
#+end_src
...
#+begin_src emacs-lisp
  (setup (:package magit)
    ...
    (when (eq system-type 'gnu/linux)
      ;; On Linux, I mostly use ZFS, so I will disable trash operations. Metadata
      ;; operations are slow on ZFS, and because ZFS supports automatic snapshots,
      ;; I can rollback the filesystem if the removed file is crucial.
      (:set magit-delete-by-moving-to-trash nil))

    ;; Improve the performance of visual line movement with `line-move-visual'.
    (:set magit-section-visibility-indicator nil
    ...
    (add-hook 'magit-post-create-buffer-hook
              (defun akirak/magit-status-run-faster ()
                (when (and (eq major-mode 'magit-status-mode)
                           (string-match-p (rx bol "nixpkgs" (syntax symbol))
                                           (file-name-nondirectory
    ...
    (keymap-set ctl-x-x-map "s"
                (defun akirak-magit-stage-file ()
                  ...
                  (when (save-window-excursion
                    ...
                    (magit-stage-files (list (buffer-file-name)))
                    (pcase (save-window-excursion
                             (magit-diff-staged)
                             (alist-get (read-char-choice "Commit the current index?")
    ...
    (with-eval-after-load 'magit-status
      ...
      ;; Remove some sections for faster magit-status runs.
      (delq 'magit-insert-am-sequence magit-status-sections-hook)
    ...
    (:status opinionated
      (:set magit-display-buffer-function
            ...
            (defun akirak/magit-display-buffer (buffer)
              (let ((action (with-current-buffer buffer
                              (pcase (derived-mode-p '(magit-diff-mode
                                                       magit-log-mode
                                                       magit-process-mode))
                                ((and (or `magit-diff-mode
                                          `magit-process-mode
                                          (and `magit-log-mode
                                               (let `(,(rx ".."))
                                                 magit-buffer-revisions)))
                                      (guard (fboundp 'akirak-window-display-buffer-prefer-other-pane)))
      ...
      (:el-patch
        (el-patch-defun magit-worktree-delete (worktree)
          (interactive
              ...
              (when (file-exists-p worktree)
                (el-patch-swap
                  ...
                  (let ((default-directory primary))
                    ;; Safely clean up the work tree
                    (when (fboundp 'akirak-process-cleanup-dir)
                      (akirak-process-cleanup-dir worktree))
                    (magit-run-git "worktree" "remove"
                                   (convert-standard-filename (expand-file-name worktree)))
    ...
    (:status personal
      (:when-loaded
        ...
        (transient-replace-suffix 'magit-merge
          'magit-merge-dissolve
          '("v" "view PR" akirak-magit-pr))
...
#+begin_src emacs-lisp
  (setup squasher
    ...
    (:nixpkgs "customPackages.squasher")
...
#+begin_src emacs-lisp
  (setup (:package marginalia)
    ...
    (:when-loaded
      (when (fboundp 'akirak-shell-ad-around-annotate-buffer)
        (advice-add 'marginalia-annotate-buffer
                    :around #'akirak-shell-ad-around-annotate-buffer))
...
#+begin_src emacs-lisp
  (setup (:package markdown-mode)
    ...
    (defun akirak/markdown-add-heading-ids ()
      "Add a unique ID to each heading in the buffer."
      (interactive)
      (save-excursion
        (goto-char (point-min))
        (let ((count 0))
          (while (< (point) (progn
            ...
            (when (looking-at (rx (+ (any "#")) (+ blank)
              ...
              (let ((content (match-string 1)))
                (unless (string-match-p (rx blank "{#" (+ nonl) "}" (* blank) eol)
                                        content)
                  (goto-char (match-end 0))
                  (let ((id (akirak/markdown-encode-id content)))
                    (when id
                      (insert (format " {#%s}" id))
                      (cl-incf count)))))))
          (when (> count 0)
            (message "Generated %d IDs" count)))))

    (defun akirak/markdown-encode-id (text)
      (with-temp-buffer
        (let (segments)
          (insert text)
          (goto-char (point-min))
          (while (re-search-forward (rx (+ alnum)) nil t)
            (push (downcase (match-string 0)) segments))
          (when segments
  ...
  (setup (:package mcp)
    ...
    (:set mcp-hub-servers
          `(("playwright-headless"
             ...
             ;; The executable is provided by nixpkgs' playwright-mcp package.
...
#+begin_src emacs-lisp
  (setup (:package mermaid-mode)
    ;; mermaid is a huge dependency (with chromium) and slow. I won't use it in
    ;; org-babel.
    ;;
    ;; (:org-babel-language t)
    ;; (:nixpkgs "mermaid-cli")
...
#+begin_src emacs-lisp
  (setup (:package move-dup)
    (:keymap "M-P"
             (defun akirak/move-dup-move-lines-up (&optional arg)
               (interactive "P")
               (if (and (bound-and-true-p treesit-primary-parser)
                        (use-region-p))
                   (akirak-treesit-swap-selected-node (if (numberp arg)
                                                          (* (-1) arg)
                                                        -1))
                 (move-dup-move-lines-up arg)))
             "M-N"
             (defun akirak/move-dup-move-lines-down (&optional arg)
               (interactive "P")
               (if (and (bound-and-true-p treesit-primary-parser)
                        (use-region-p))
                   (akirak-treesit-swap-selected-node (if (numberp arg)
...
*** nael
:PROPERTIES:
:CREATED_TIME: [2025-10-16 Thu 20:12]
:END:
#+begin_quote
Major mode for Lean
...
#+begin_src emacs-lisp
  (setup (:package nael)
    (:file-match "\\.lean\\'")
    (:mode-project-roots "lakefile.toml" "lakefile.lean")
    (:set nael-prepare-eglot t
          nael-prepare-lsp nil)
    (:keymap "C-c C-r" #'akirak/lake-lean-run))
#+end_src
*** neotree
:PROPERTIES:
:CREATED_TIME: [2025-08-14 Thu 16:18]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package neotree)
    (:set neo-window-width 40
          neo-theme 'nerd-icons
          neo-show-updir-line nil
          neo-vc-integration nil)
    (:local-set line-spacing 0.2)
    (:global "C-z f" #'neotree-find)

    ;; Use a smaller font size for tree entries
    (:when-loaded
      ...
      (set-face-attribute 'neo-vc-ignored-face nil
                          ...
                          :strike-through nil)
      (cl-do-all-symbols (sym)
        (when (and (string-match-p (rx bol "neo-"
                                       (or (and "vc-" (+ anything))
                                           (and (+ anything)
                                                "-link-face"))
                                       eol)
                                   (symbol-name sym))
                   (facep sym))
          (set-face-attribute sym nil :height 0.92)))))
#+end_src
*** nerd-icons
:PROPERTIES:
:CREATED_TIME: [2025-08-14 Thu 22:41]
:END:
#+begin_quote
  ...
  (setup (:package ocp-indent)
    (defun akirak/ocp-indent-maybe-setup ()
      (when (executable-find "ocp-indent")
        (ocp-setup-indent)))
    (add-hook 'tuareg-mode-hook #'akirak/ocp-indent-maybe-setup)
    (add-hook 'neocaml-mode-hook #'akirak/ocp-indent-maybe-setup))
#+end_src
*** olivetti                                               :writing:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package olivetti)
    ...
    (:global-keymap "S-<f11>" #'olivetti-mode)
    ...
    (advice-add 'olivetti-set-window
                :after
                (defun akirak/ad-after-olivetti-set-window (window-or-frame)
                  (when (windowp window-or-frame)
                    (require 'akirak-window)
                    (akirak-window-set-centered-margins window-or-frame)))))
#+end_src
...
*** protobuf-mode
:PROPERTIES:
:CREATED_TIME: [2026-02-19 Thu 23:59]
:END:
#+begin_quote
...
#+end_src
*** COMMENT rich-minority                                                   :UI:ARCHIVE:
:PROPERTIES:
...
*** rime                                                        :Chinese:@extra:
:PROPERTIES:
:CREATED_TIME: [2026-05-05 Tue 15:01]
:END:
#+begin_quote
Rime input method
...
#+begin_src emacs-lisp
  (setup (:package rime)
    (:when-loaded
      ;; You have to set `rime-share-data-dir' to a directory that contains rime
      ;; dictionaries.
      (:set rime-share-data-dir (file-name-concat (xdg-data-home) "rime-data"))
...
#+begin_src emacs-lisp
  (setup (:package string-inflection)
    (defconst akirak/string-inflection-functions
    ...
    (defun akirak/string-inflection (begin end)
      (let* ((orig (buffer-substring begin end))
             (alist (thread-last
                      akirak/string-inflection-functions
                      (mapcar `(lambda (func)
                                 (cons (symbol-name func)
                                       (funcall func ',orig))))))
             (annotation-function (lambda (cand)
                                    (concat " " (cdr (assoc cand alist)))))
             (name (completing-read (format "Convert \"%s\": " orig)
...
#+begin_src emacs-lisp
  (setup (:package super-save)
    ...
    ;; Prevent committing buffers too early, especially for edit-indirect.
    (advice-add 'super-save-maybe-silently
                :before-while
                (defun akirak/ad-before-super-save-maybe-silently (&rest _args)
                  (and buffer-display-time
                       (> (- (float-time)
...
**** COMMENT casual-symbol-overlay                                     :transit:ARCHIVE:
:PROPERTIES:
...
*** tab-bookmark
:PROPERTIES:
:CREATED_TIME: [2025-05-05 Mon 12:24]
:END:
#+begin_quote
Tab bookmarks
...
(setup (:package taxy))
#+end_src
*** COMMENT taxy-magit-section                                      :UI:library:ARCHIVE:
:PROPERTIES:
:CREATED_TIME: [2022-02-02 Wed 23:18]
:END:
#+begin_quote
...
:LOGBOOK:
CLOCK: [2026-09-16 Wed 10:40]--[2026-09-16 Wed 11:07] =>  0:27
:END:
#+begin_quote
Code folding using treesit
[[https://github.com/emacs-tree-sitter/treesit-fold]]
...
#+end_src
*** COMMENT tuareg                                          :OCaml:@extra:fstar:ARCHIVE:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package tuareg)
    ...
    (:when-loaded
      ...
      (define-abbrev tuareg-mode-abbrev-table
        "struct" "" (define-skeleton akirak-ocaml-struct
                      "Insert struct"
                      ""
                      > "struct" n
                      > @ - n
      ...
      (define-abbrev tuareg-mode-abbrev-table
        "sig" "" (define-skeleton akirak-ocaml-sig
                   "Insert sig"
                   ""
                   > "sig" n
                   > @ - n
...
*** unfill
:PROPERTIES:
:CREATED_TIME: [2025-05-17 Sat 12:40]
:END:
#+begin_quote
...
#+begin_src emacs-lisp
(setup (:package unfill))
#+end_src
*** COMMENT uniline                                                    :drawing:ARCHIVE:
:PROPERTIES:
:CREATED_TIME: [2025-11-10 Mon 23:15]
:END:
#+begin_quote
...
#+end_quote
*** video-trimmer                                                 :media:@extra:
:PROPERTIES:
:CREATED_TIME: [2025-08-22 Fri 12:09]
:END:
#+begin_quote
-*- lexical-binding: t -*-
#+end_quote
...
#+begin_src emacs-lisp
  (setup (:package whole-line-or-region)
    ...
    (:global-keymap "C-c r" #'akirak/anzu-query-replace-on-line)
...
#+begin_src elisp
  (setup gradle
    (:package-files "build.gradle.kts"))
#+end_src
*** Haskell
:PROPERTIES:
...
#+begin_src elisp
  (setup lake
    (:package-files "lakefile.toml"
                    "lakefile.lean")

    (defun akirak/lake-lean-run (file &optional arg)
      (interactive (list (buffer-file-name)
                         arg))
      (let ((default-directory (or (locate-dominating-file
                                    file (lambda (dir)
                                           (when (file-directory-p dir)
                                             (seq-intersection (directory-files dir)
                                                               '("lakefile.lean"
                                                                 "lakefile.toml")))))
                                   (user-error "Found neither lakefile.lean nor leanfile.toml"))))
        (compile (read-string "Run: "
                              (format "lake env lean --run %s"
                                      (shell-quote-argument (file-relative-name file))))
                 comint))))
#+end_src
*** Mix (Elixir)
#+begin_src elisp
...
#+begin_src elisp
  (setup uv
    ...
    (defun akirak/euporie-project ()
      (interactive)
      (let* ((root (locate-dominating-file default-directory "pyproject.toml")))
        (akirak-shell-exec-in-project (list "uv" "run" "euporie-console")
  ...
  (add-hook 'org-pivot-search-entry-display-hook
            (defun akirak/org-pivot-search-after-display ()
              ...
              (if (buffer-narrowed-p)
                  (akirak/org-recenter-toplevel-at-top)
                (recenter))))
  (setq org-rooted-find-hook #'akirak/org-goto-indirect-hook)
  ...
  (setup init
    (:status personal
      ...
      (when (and custom-file (file-readable-p custom-file))
        (load custom-file))))
#+end_src
** Integrating into NixOS/Home-Manager
This configuration is built with Nix (via [[https://github.com/emacs-twist/twist.nix][twist.nix]]) and integrated into
home-manager.

I am currently trying to adopt [[https://github.com/yunfachi/denix][denix]] for NixOS configurations, and [[file:nix/denix/modules/emacs/default.nix]] contains a denix module for integration.
It depends on the home-manager module provided from twist.nix.

[[file:nix/denix/modules]] contains other denix modules mostly for home-manager.
Not all of my denix modules are defined there; There are other things that are kept private. The modules make some assumptions on the denix configuration. Refer to the entire denix configurations in [[file:nix/denix]] and [[file:flake.nix]] for home-manager integration.

To use the modules from another repository, this flake exposes some outputs.
The following is an example configuration with the ~emacs-config~ input from this repository:
  ...
  let
    mkConfigurations = moduleSystem: denix.lib.configurations {
      ...
      paths = [
        ./hosts
        ./modules
        # Import the modules from https://github.com/akirak/emacs-config/
        inputs.emacs-config.lib.denixModules
      ];
      ...
      specialArgs = {
        inherit
          moduleSystem
          ;
        # Merge the inputs from https://github.com/akirak/emacs-config/
        inputs = inputs.emacs-config.lib.denixInputs // inputs;
        # Use the overlays from https://github.com/akirak/emacs-config/
        overlays = [
          inputs.emacs-config.overlays.default
          inputs.emacs-config.overlays.extras
        ];
      };
    };
  in {
...
| [[org-ql-search:tags:packages][packages]]            | 218 |
...
| [[org-ql-search:tags:library][library]]             |   6 |
...
| [[org-ql-search:tags:drawing][drawing]]             |   2 |
...
| [[org-ql-search:tags:Chinese][Chinese]]             |   2 |
...
| [[org-ql-search:tags:media][media]]               |   1 |
...
C-a, C-e, C-u, C-w, C-h.
** Also see
- [[https://github.com/terlar/emacs-config][Tarje Larsen's emacs-config]] is a configuration from which this configuration takes a constant inspiration.
- The denix modules are partly based on [[https://github.com/yunfachi/nix-config][yunfachi's nix-config]].
** License
