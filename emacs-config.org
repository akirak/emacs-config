...
#+begin_src emacs-lisp
  (setup (:package gptel)
    (defconst akirak/ai-model-list-file
      (locate-user-emacs-file "ai-model-list.txt"))

    (defun akirak/gptel-openrouter-models ()
      (with-temp-buffer
        (insert-file-contents akirak/ai-model-list-file)
        (delete-non-matching-lines (rx bol "openrouter/"))
        (thread-last
          (string-split (buffer-string) "\n")
          (mapcar (apply-partially #'string-remove-prefix "openrouter/"))
