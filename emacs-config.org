...
#+begin_src emacs-lisp
  (setup completion-preview
    ...
    (:with-map completion-preview-active-mode-map
      (:bind "C-j" #'completion-preview-insert)))
#+end_src
*** custom
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup simple
    ...
    (when (version< emacs-version "31")
      (:global "C-w" (defun akirak/kill-region-or-backward-kill-word (&optional arg)
...
#+begin_src emacs-lisp
  (setup smerge
    (:global "C-c g u" #'smerge-keep-upper
...
#+begin_src emacs-lisp
  (setup (:package org)
    (:with-mode org-mode
      ...
      (:global "M-g o" #'org-open-at-point-global)
...
#+begin_src emacs-lisp
  (setup org
    (:with-mode org-mode
      ...
      (:bind "C-a" #'org-beginning-of-line
             "C-e" #'org-end-of-line)
...
#+begin_src emacs-lisp
  (setup (:package org-hyprctl)
    ...
    (:bind "C-M-n" #'move-dup-move-lines-down
...
#+begin_src emacs-lisp
  (setup akirak-bitwarden
    (:with-mode akirak-bitwarden-rbw-mode
      (:file-match "rbw\\'"))
    (:global "<f12> b g" #'akirak-bitwarden-show
...
#+begin_src emacs-lisp
  (setup akirak-consult
    (:status opinionated
      (:global [remap consult-imenu-multi] #'akirak-consult-imenu-multi)
      (:with-map ctl-x-map
        (:bind "p" #'akirak-consult-project-file))))
#+end_src
...
#+begin_src elisp
  (setup akirak-import
    (:global "C-c i" #'akirak-import-thing-at-pt))
#+end_src
*** akirak-insert
#+begin_src emacs-lisp
  (setup akirak-insert
...
#+begin_src emacs-lisp
  (setup akirak-org-clock
    ...
    (with-eval-after-load 'org-keys
      (define-key org-mode-map [remap org-clock-in] #'akirak-org-clock-in))
...
#+begin_src elisp
  (setup akirak-paren
    ...
    (:status opinionated
      (:global "M-i c" #'akirak-paren-replace
...
#+begin_src elisp
  (setup akirak-rename
    (:global "C-c r" #'akirak/rename-dwim)
...
#+begin_src elisp
  (setup akirak-treesit
    (:bind [remap akirak-expand-region-default] #'akirak-treesit-expand-region)
...
#+begin_src emacs-lisp
  (setup (:package apheleia)
    (:when-loaded
      ...
      ;; Make apheleia-npx available.
      (add-to-list 'exec-path
                   (expand-file-name "scripts/formatters"
                                     (file-name-directory (find-library-name "apheleia"))))
        ...
        (with-current-buffer scratch
          (let ((default-directory (expand-file-name
            ...
            (call-process-region (point-min) (point-max)
                                 "apheleia-npx"
                                 t t nil
                                 "eslint" "--fix-dry-run" "-f" "json"
                                 "--stdin"
                                 "--stdin-filename"
                                 (file-relative-name (buffer-local-value 'buffer-file-name buffer)))
            (goto-char (point-min))
...
#+end_src
*** COMMENT copilot                                             :@extra:copilot:ARCHIVE:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package cosmetic-transient)
    (:global "C-c m" #'cosmetic-transient))
#+end_src
*** csv-mode
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package d2-mode)
    ...
    (:bind "C-c C-w" #'akirak/d2-watch-file)
...
#+begin_src emacs-lisp
  (setup (:package diff-hl)
    ...
    (add-hook 'akirak-worktree-file-mode-hook #'akirak/turn-on-envrc-mode)
...
#+begin_src emacs-lisp
  (setup (:package eat)
    ...
    (:with-map eat-semi-char-mode-map
      (:bind "S-<prior>" #'View-scroll-page-backward
             "S-<next>" #'View-scroll-page-forward)
      (:unbind "M-:" "M-g" "M-s" "C-z" "M-o" "M-'")))
#+end_src
*** ediprolog                                                           :Prolog:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package embark)
    ...
    (define-key minibuffer-mode-map (kbd "C-SPC") #'embark-select)
...
#+begin_src emacs-lisp
  (setup (:package github-linguist)
    ...
    (:global "<f12> g l" #'github-linguist-switch-project-by-language))
#+end_src
*** git-identity                                                    :VC:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package gptel)
    ...
    (:global "C-c p" #'gptel-menu)

    (with-eval-after-load 'embark
      (define-key embark-region-map "!" #'gptel-rewrite-menu))
...
#+begin_src emacs-lisp
  (setup (:package magit)
    ...
    (:with-map (magit-status-mode-map magit-log-mode-map)
      (:bind "^" (defun akirak/magit-up-directory ()
...
#+begin_src emacs-lisp
  (setup (:package minuet)
    (:global "C-c y" #'minuet-complete-with-minibuffer)
    (:with-map minuet-active-mode-map
      (:bind "<M-return>" #'minuet-accept-suggestion))
    ;; (:set minuet-provider 'openai-fim-compatible
    ;;       ;; recommended for Local LLM for resource saving
    ;;       minuet-n-completions 1
    ;;       ;; I recommend you start with a small context window firstly,
    ;;       ;; and gradually increase it based on your local computing power.
    ;;       minuet-context-window 512)
    (:when-loaded
      (plist-put minuet-claude-options
                 :api-key
                 (defun akirak/minuet-anthropic-api-key ()
                   (require 'gptel)
                   (gptel-api-key-from-auth-source "api.anthropic.com")))
      (plist-put minuet-openai-fim-compatible-options
                 :api-key
                 (defun akirak/minuet-deepseek-api-key ()
                   (require 'gptel)
...
#+begin_src emacs-lisp
  (setup (:package casual-symbol-overlay)
    (:global "C-c S" #'casual-symbol-overlay-tmenu))
#+end_src
*** systemd
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package treesit-fold)
    ...
    (:bind "C-c TAB" #'akirak-treesit-fold-dwim))
#+end_src
