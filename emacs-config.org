...
#+begin_src emacs-lisp
  (setup eglot
    ...
    (:when-loaded
      ;; This hook has become built into eglot, but it slows down loading of
      ;; buffers where I don't need LSP integration especially due to calling
      ;; `project-current'. Will opt out.
