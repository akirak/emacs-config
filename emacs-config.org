...
#+begin_src elisp
  (setup akirak-shell
    (:global-keymap "C-c t" #'akirak-shell-transient))
#+end_src
*** akirak-shr
#+begin_src elisp
  (setup akirak-shr
...
#+begin_src emacs-lisp
  (setup (:package copilot)
    ;; Unfree package which fails to build on CI. Add it to home.packages instead
...
**** sqlglot
:PROPERTIES:
:CREATED_TIME: [2025-04-07 Mon 11:47]
:END:

#+begin_src emacs-lisp
  (setup sqlglot
    (:nixpkgs sqlglot-formatter)

    (defcustom sqlglot-formatter-dialect "postgres"
      "SQL dialect used in the buffer."
      :type '(choice (const nil)
                     (const "Name of a dialect supported by sqlglot" string)))

    ;; sqlglot-formatter is a tiny custom package that wraps sqlglot.
    (:reformatters
     (sqlglot-format
      :program "sqlglot-formatter"
      :args (list "--dialect" "postgres"))))
#+end_src
