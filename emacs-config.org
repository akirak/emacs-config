...
#+begin_src emacs-lisp
  (setup json-ts-mode
    (:file-match "\\.json\\'" "\\.lock\\'" "*.webmanifest")
...
#+begin_src emacs-lisp
  (setup akirak-org-clock
    ...
    (add-hook 'org-clock-in-hook
              (defun akirak/org-clock-restore-burly ()
                (let ((url (org-entry-get nil "burly_windows_url"))
                      (datetime (org-entry-get nil "burly_saved_at")))
                  (when (and url
                             (not (string-empty-p url))
                             (require 'burly nil t)
                             (yes-or-no-p (format-prompt
    ...
    (add-hook 'akirak-org-clock-pre-exit-hook
              (defun akirak/org-clock-save-burly ()
                (when (require 'burly nil t)
...
#+begin_src emacs-lisp
  (setup (:package envrc)
    (defun akirak/turn-on-envrc-mode ()
      ;; Disable envrc for performance.
      (unless (or (minibufferp)
                  (string-match-p "\*consult-" (buffer-name)))
        (envrc-mode 1)))
