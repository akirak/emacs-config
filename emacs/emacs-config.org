...
#+begin_src emacs-lisp
  (setup browse-url
    (:status personal
      ...
      (defun akirak/browse-url-external-browser (url &optional _arg)
        (let ((host (url-host (if (url-p url)
                                  url
                                (url-generic-parse-url url)))))
          (if (member host '("airtable.com"))
...
#+begin_src emacs-lisp
  (setup (:package orgabilize)
    ...
    (with-eval-after-load 'embark
      (define-key embark-url-map (kbd "r") #'orgabilize-org-archive)))
#+end_src
*** org-autolist
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package akirak)
    ...
    (:global "C-c c e" #'akirak-emacs-config-capture)
...
#+begin_src emacs-lisp
  (setup akirak-url
    ...
    (with-eval-after-load 'embark
      ...
      (define-key embark-file-map (kbd "C-l") #'akirak-org-store-link-to-file)
      (define-key embark-url-map (kbd "c") #'akirak-url-org-capture)))
#+end_src
...
A recommended way to turn on automatic commits is to adding to =.dir-locals.el=.
*** git-gutter                                                         :VC:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package mlscroll)
    (:set mlscroll-in-color (face-attribute 'region :background nil t)
...
#+begin_src emacs-lisp
  (setup (:package svelte-mode)
    (:file-match "\\.svelte\\'")
    (defvar akirak/svelte-remap-submodes-done nil)
    (:hook
     (defun akirak/svelte-remap-submodes ()
       "Use `puni-mode' only inside submodes."
       (require 'puni)
       (puni-mode -1)
       (unless akirak/svelte-remap-submodes-done
         (save-excursion
           (goto-char (point-min))
           (let (match
                 names)
             (while (setq match (text-property-search-forward 'svelte-submode))
               (when-let (submode (prop-match-value match))
                 (unless (member (svelte--submode-name submode) names)
                   (setf (svelte--submode-keymap submode)
                         (make-composed-keymap (list puni-mode-map)
                                               (svelte--submode-keymap submode)))
                   (push (svelte--submode-name submode) names))))
             names))
         ;; Prevent creating keymaps every time `svelte-mode' is called
         (setq-local akirak/svelte-remap-submodes-done t))))
...
| [[org-ql-search:tags:packages][packages]]            | 173 |
...
| [[org-ql-search:tags:reading][reading]]             |   3 |
