  ...
  (setup project
    ...
    (:status opinionated
      (:when-loaded
        ...
        (add-hook 'project-find-functions #'akirak-project-find-subdir-root)
          ...
          (defun akirak/tab-bar-tab-name ()
            ...
            (let* ((buffer (window-buffer (or (minibuffer-selected-window)
              ...
              (if (or (not pr)
                  ...
                  (buffer-name buffer)
                (format "%s:%s"
                        (truncate-string-to-width (project-name pr) 12)
                        (if (eq 'dired-mode (buffer-local-value 'major-mode buffer))
                            (file-relative-name dir (project-root pr))
                          (buffer-name buffer))))))))
#+end_src
*** tempo
:PROPERTIES:
...
*** nael
:PROPERTIES:
:CREATED_TIME: [2024-06-18 Tue 19:40]
:END:
#+begin_quote
...
I prefer [[#zig-ts-mode][zig-ts-mode]], but I install ~zig-mode~ for build commands.
