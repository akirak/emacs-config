      ...
      (advice-add 'mailcap-parse-mailcaps
                  ...
                  (defun akirak/ad-around-mailcap-parse-mailcaps (orig &rest args)
                    (if (and (not args)
                        ...
                        (or (akirak-mime-set-mailcap-extensions)
                            (funcall orig))
                      (apply orig args))))))
#+end_src
*** menu-bar
  ...
  (setup timer
    ...
    (run-with-idle-timer 240 nil
                         (defun akirak/save-all-buffers-non-interactively ()
                           ...
                           (let ((save-some-buffers-default-predicate nil))
   ...
   `org-src-window-setup' is \='current-window, but it is %s"
    ...
    (cl-macrolet
      ...
      (dolist (entry `((,(major-modes help-mode devdocs-mode)
        ...
        (add-to-list 'display-buffer-alist entry t)))
...
#+begin_src elisp :results value code :tangle no
  (cl-flet*
      ((lib-language-name (filename)
         (when (string-match (rx-to-string
                              `(and bol "libtree-sitter-"
                                    (group (+ anything))
                                    ,(car dynamic-library-suffixes) eol))
                             filename)
           (match-string 1 filename)))
       (get-languages (dir)
         (thread-last
           (directory-files dir)
           (mapcar #'lib-language-name)
           (delq nil)))
       (to-mapping (lang)
         (let* ((ts-sym (intern-soft (concat lang "-ts-mode")))
                (non-ts-sym (intern-soft (concat lang "-mode"))))
           (when (and ts-sym (commandp ts-sym))
             (cons lang (when (commandp non-ts-sym)
                          non-ts-sym))))))
    (let ((mappings (thread-last
                     treesit-extra-load-path
                     (mapcar #'get-languages)
                     (flatten-list)
                     (mapcar #'to-mapping)
                     (delq nil))))
      `(progn
         (let ((remappings ',(thread-last
                               mappings
                               (cl-remove-if-not #'cdr)
                               (mapcar (lambda (pair)
                                         (cons (cdr pair)
                                               (intern (concat (car pair) "-ts-mode"))))))))
           (cl-flet
               ((modify-entry (pair)
                  (if-let (remapping (assq (cdr pair) remappings))
                      (cons (car pair) (cdr remapping))
                    pair)))
             (dolist (var-sym '(auto-mode-alist
                                interpreter-mode-alist))
               (set var-sym (mapcar #'modify-entry (symbol-value var-sym))))))
         (with-eval-after-load 'org
           (dolist (pair ',(thread-last
                             mappings
                             (mapcar #'car)
                             (mapcar (lambda (lang)
                                       (cons lang (intern (concat lang "-ts")))))))
...
#+begin_src elisp
  (progn
    (let
        ((remappings
          '((c-mode . c-ts-mode) (css-mode . css-ts-mode)
            (html-mode . html-ts-mode) (java-mode . java-ts-mode)
      ...
      (cl-flet
          ((modify-entry (pair)
             (if-let (remapping (assq (cdr pair) remappings))
                 (cons (car pair) (cdr remapping))
               pair)))
        (dolist (var-sym '(auto-mode-alist interpreter-mode-alist))
          (set var-sym (mapcar #'modify-entry (symbol-value var-sym))))))
    (with-eval-after-load 'org
      (dolist
          (pair
           '(("bash" . bash-ts) ("c" . c-ts) ("cmake" . cmake-ts) ("css" . css-ts)
             ("dockerfile" . dockerfile-ts) ("elixir" . elixir-ts) ("go" . go-ts)
             ("heex" . heex-ts) ("html" . html-ts) ("java" . java-ts)
             ("json" . json-ts) ("python" . python-ts) ("ruby" . ruby-ts)
             ("rust" . rust-ts) ("toml" . toml-ts) ("tsx" . tsx-ts)
             ("typescript" . typescript-ts) ("yaml" . yaml-ts)))
        (add-to-list 'org-src-lang-modes pair))))
#+end_src
...
#+begin_src emacs-lisp
  (setup embark-org
    (eval-after-load 'org #'akirak-embark-setup-org))
#+end_src
