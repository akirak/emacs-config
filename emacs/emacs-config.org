  ...
  (setup (:package org-dog)
    ...
    (with-eval-after-load 'recentf
      (add-to-list 'recentf-exclude
                   (concat (regexp-opt (mapcar #'car org-dog-repository-alist) :paren)
                           (rx (+ anything) ".org" eol))))
    ...
    (:status personal
      (add-hook 'org-dog-clock-in-success-hook
                (defun akirak/org-dog-add-git-properties-on-clock-in (pom)
                  ...
                  (let ((element (org-with-point-at pom
                    ...
                    (unless (org-element-property :GIT_ORIGIN element)
                   ...
                   (cons 'org-clock
                         (defun akirak/dashboard-org-clock-insert (list-size)
                           (dashboard-insert-section
                            "Org Clock (unfinished items)"
                            (when (boundp 'org-clock-history)
                              (require 'akirak-org-clock)
                              (akirak-org-clock--dashboard-items list-size))
                            list-size
                            'org-clock
                            (dashboard-get-shortcut 'org-clock)
                            `(lambda (&rest _)
                               (require 'org-dog)
                               (pop-to-buffer (org-dog-indirect-buffer
                                               (org-element-property :hd-marker ',el))))
                            (akirak-org-clock--format-dashboard-item el))))))
    ...
    (when (daemonp)
      (add-hook 'after-init-hook
                (defun akirak/dashboard-init-on-startup ()
                  ...
                  (add-hook 'server-after-make-frame-hook #'dashboard-initialize))
                ;; It should not precede over bookmark-frecency-mode
                85)))
#+end_src
