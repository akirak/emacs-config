...
#+begin_src elisp
  (setup gleam
    ...
    (defun akirak/gleam-shell ()
      ...
      (if-let* ((root (locate-dominating-file default-directory "gleam.toml"))
                (pr (project-current))
                (name (format "gleam-%s" (project-name pr)))
                (default-directory root))
          (pop-to-buffer (eat-make name "gleam" nil "shell"))
  ...
  (setup dune
    ...
    (defun akirak/utop-project ()
      ...
      (let* ((root (locate-dominating-file default-directory "dune-project"))
        ...
        (pop-to-buffer (apply #'eat-make name
                              (if root
                                  (append (if (and (not (executable-find "dune"))
                                                   (executable-find "opam"))
                                              '("opam" nil "exec" "--" "dune" "utop")
                                            '("dune" nil "utop"))
                                          (list (shell-quote-argument (file-relative-name root))))
                                (cond
