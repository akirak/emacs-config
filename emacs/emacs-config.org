...
*** dir-locals
:PROPERTIES:
:CREATED_TIME: [2023-02-19 Sun 09:44]
:END:
  ...
  (setup dir-locals
    (:status personal
      ...
      (dir-locals-set-class-variables
       'web
       (mapcar (lambda (mode)
                 `(,mode . ((eval . (prettier-on-save-mode)))))
               '(web-mode
                 css-mode
                 typescript-ts-mode)))
      (dir-locals-set-directory-class "~/work2/personal/" 'default)
      (dir-locals-set-directory-class "~/work2/personal/" 'web)))
#+end_src
  ...
  (setup frame
    ...
    (setq frame-title-format '("%b - "
                               (akirak/emacs-profile-name
                                akirak/emacs-profile-name
                                (:eval (concat "GNU Emacs " emacs-version))))))
#+end_src
*** help
:PROPERTIES:
:CREATED_TIME: [2022-09-03 Sat 23:59]
...
#+begin_src emacs-lisp
  (setup server
    (when window-system
      (defcustom akirak/server-start-hook nil "" :type 'hook)

      (defun akirak/server-start ()
        (server-start)
        (run-hooks 'akirak/server-start-hook))
      ...
      (:status personal
        (add-hook 'emacs-startup-hook #'akirak/start-server-at-startup))))
#+end_src
*** sgml-mode
:PROPERTIES:
...
Following sections in this configuration file assume my extras are available.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (setup akirak-org-clock
    (:hook-into akirak/server-start-hook)
...
#+begin_src emacs-lisp
  (setup (:package elixir-mode)
    ...
    (:when-loaded
      (define-abbrev elixir-mode-abbrev-table
...
#+begin_src emacs-lisp
  (setup (:package git-auto-commit-mode)
    ...
    (with-eval-after-load 'dir-locals
      (dir-locals-set-class-variables
       'git-auto-commit
    ...
    (defun akirak/git-auto-commit-message (filename)
      (let ((root (project-root (project-current nil (file-name-directory filename)))))
        (thread-last
          (if (org-clocking-p)
...
#+begin_src emacs-lisp
  (setup (:package nix3)
    (:set nix3-flake-worktree-promise-fn #'akirak/nix3-flake-find-repo)

    (defconst akirak/nix3-registry-url
      "https://raw.githubusercontent.com/akirak/flake-pins/master/registry.json")

    (defun akirak/nix3-flake-lock-from-pins ()
      (compile (mapconcat #'shell-quote-argument
                          `("nix" "flake" "lock"
                            "--registry" ,akirak/nix3-registry-url)
                          " ")))

    (:when-loaded
      (add-hook 'nix3-flake-new-hook #'akirak/nix3-flake-lock-from-pins
                nil t))

    (defun akirak/nix3-flake-find-repo (alist)
      (require 'akirak-git-clone)
      (let-alist alist
        (let ((dir (expand-file-name
                    (pcase-exhaustive \.type
                      ("github"
                       (format "github.com/%s/%s" \.owner \.repo))
                      ("gitlab"
                       (format "gitlab.com/%s/%s" \.owner \.repo))
                      ("sourcehut"
                       (format "git.sr.ht/%s/%s" \.owner \.repo))
                      ("git"
                       (let ((url (url-generic-parse-url \.url)))
                         (format "%s/%s" (url-host url) (url-filename url)))))
                    "~/archives/oss/"))
              (origin (if (equal \.type "git")
                          \.url
                        (thread-last
                          (nix3-flake-ref-alist-to-url alist)
                          (akirak-git-clone--parse)
           ...
           `(lambda (resolve _)
              (let ((dir ,dir)
                ...
                (if (file-directory-p dir)
                  ...
                  (akirak-git-clone--clone origin dir :callback resolve))))))))

    (:global "C-c n" #'nix3-transient
             "M-g M-f" #'nix3-flake-edit
             "M-s M-n" #'nix3-registry-list))
#+end_src
**** magit-nix3
#+begin_quote
...
#+begin_src emacs-lisp
  (setup (:package reformatter)
    (:reformatters
     ...
     (treefmt
      :program "treefmt"
      :stdin nil :stdout nil :mode nil
      :args (list (buffer-file-name))))

    (define-minor-mode treefmt-mode
      "Run treefmt after saving."
      :global nil
      :lighter " Treefmt"
      (if (bound-and-true-p treefmt-mode)
          (add-hook 'after-save-hook 'treefmt-buffer 90 t)
        (remove-hook 'after-save-hook 'treefmt-buffer t))))
#+end_src
