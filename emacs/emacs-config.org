# -*- org-id-link-to-org-use-id: nil; org-complete-tags-always-offer-all-agenda-tags: nil; -*-
...
*** server
:PROPERTIES:
:CREATED_TIME: [2022-10-15 Sat 13:04]
:END:
  ...
  (setup server
    (when window-system
      (add-hook 'emacs-startup-hook
                (defun akirak/start-server-at-startup ()
                  (require 'server)
                  (if (server-running-p)
                      ;; Try to start an Emacs server again in 3 minutes.
                      (run-with-timer (* 3 60) nil
                                      (lambda ()
...
#+begin_src emacs-lisp
  (setup (:package oahu)
    (:status personal
      ...
      (:global "<S-menu>" #'oahu-alternative-view)))
#+end_src
**** oahu-memento
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package oahu-memento)
    (:status personal
      ...
      (define-key ctl-x-map "g" #'akirak/oahu-switch-to-viewed-project)

      (defun akirak/oahu-viewed-project ()
        (pcase oahu-last-view
          (`(Project ,pr . ,_)
           pr)))
      (defun akirak/oahu-switch-to-viewed-project ()
        (interactive)
        (when-let (pr (akirak/oahu-viewed-project))
  ...
  (setup org-keys
    (add-hook 'org-follow-link-hook
              (defun akirak/org-follow-link-reveal ()
                ;; For performance, ignore derived modes for now
    ...
    (define-key help-map (kbd "M-a")
                (defun akirak/org-default-agenda ()
                  ...
                  (unless (bound-and-true-p org-clock-history)
                    (akirak-org-clock-add-agenda-files))
...
#+begin_src emacs-lisp
  (setup org-clock
    (:set org-clock-history-length 50
          ...
          org-clock-frame-title-format '((org-memento-current-block
...
#+begin_src emacs-lisp
  (setup akirak-org-clock
    ...
    (define-key akirak/c-z-map "I" #'akirak-org-clock-in-to-project)

    ;; Bindings that don't end the transient map
    ...
    (put 'akirak-org-clock-out 'repeat-map 'akirak/c-z-map)
    (put 'akirak-org-clock-done 'repeat-map 'akirak/c-z-map)
    (put 'akirak-org-clock-set-review 'repeat-map 'akirak/c-z-map)

    (:with-feature org
      (:with-map org-mode-map
        (:bind "C-c C-x S" #'akirak-org-clock-snooze))))
#+end_src
**** org-habit
:PROPERTIES:
              ...
              (defun akirak/org-dog-add-extra-agenda-files ()
                ...
                (setq org-agenda-files
                      (thread-last
                        ...
                        (mapcar #'car)
                        (reverse)))
                (when (require 'org-memento nil t)
...
[[https://git.sr.ht/~akirak/org-memento]]
#+end_quote
...
#+begin_src emacs-lisp
  (setup (:package org-memento)
    ...
    (:status personal
      ...
      (add-hook 'org-memento-checkout-hook #'save-some-buffers)
      (add-hook 'org-memento-checkin-hook
                (defun akirak/org-memento-insert-clocktable ()
                  "Insert a clock table for the day starting at the time."
                  (insert "#+BEGIN: clocktable :scope agenda"
                          " :tstart \""
                          (format-time-string (org-time-stamp-format t))
                          "\" :tend \"<now>\"\n"
                          "#+END:\n")))
      (add-hook 'org-memento-checkin-hook
                (defun akirak/org-memento-set-frame-title ()
                  (setq frame-title-format
                        '((org-memento-current-block
                           org-memento-frame-title-format
                           ;; akirak/org-memento-fallback-frame-title
                           )))))
      (add-hook 'org-memento-block-start-hook
                (defun akirak/org-memento-set-agenda-files ()
                  (setq akirak-org-dwim-selected-files (org-memento-agenda-files))))
      (add-hook 'org-memento-block-exit-hook
                (defun akirak/org-memento-cleanup-buffers ()
                  ;; Kill all indirect buffers
                  (when-let (buffers (thread-last
                                       (buffer-list)
...
#+begin_src emacs-lisp
  (setup (:package org-super-links)
    (:set org-super-links-default-description-formatter
          (defun akirak/org-super-links-description (link orig-desc)
            (if-let (file (save-match-data
                            (and (string-match (rx bos "id:" (group (+ anything)))
                                               link)
                                 (org-id-find-id-file (match-string 1 link)))))
                (format "[%s] %s" (file-name-base file) orig-desc)
              orig-desc))))
#+end_src
*** org-transclusion
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup akirak-nix
    (:global "<f12> n p" #'akirak-nix-browse-nixpkgs))
#+end_src
**** akirak-snippet
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (setup akirak-unknown
    ...
    (:with-map akirak-unknown-text-map
      (:bind "s" #'org-memento-start-quick-event)
      (:bind "a" #'org-memento-add-quick-event)))
#+end_src
...
#+begin_src emacs-lisp
  (setup (:package magit-section)
    (:with-map magit-section-mode-map
      (:bind "C-c =" (defun akirak/magit-show-section-value-at-point ()
                       (interactive)
                       (message (prin1-to-string (oref (magit-current-section) value)))))
...
#+begin_src emacs-lisp
  (setup (:package popper)
    ...
    (:bind "M-\"" #'popper-cycle
           ...
           "M-' s" #'scratch-buffer
           "M-' b" (defun akirak/backtrace-buffer ()
                     (interactive)
                     (akirak/popper-display-buffer "*Backtrace*"))
...
#+begin_src emacs-lisp
  (setup (:package rich-minority)
    (:set rm-whitelist (rx (or "envrc" "OrgMemento")))
    (:hook-into after-init-hook))
#+end_src
*** ruby-end
:PROPERTIES:
...
#+begin_src emacs-lisp
  (setup (:package workbox)
    (:global "C-c p" #'workbox-run-some-package-manager
             "C-c SPC" #'workbox-transient)
...
| [[org-ql-search:tags:library][library]]             |   9 |
| [[org-ql-search:tags:editing][editing]]             |   7 |
| [[org-ql-search:tags:completion][completion]]          |   7 |
...
| [[org-ql-search:tags:major_mode][major_mode]]          |   4 |
...
| [[org-ql-search:tags:@extra][@extra]]              |   2 |
