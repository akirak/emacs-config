...
#+begin_src emacs-lisp
  (setup register
    ...
    (:global "M-SPC" #'akirak/register-dwim)
    ...
    (add-hook 'org-follow-link-hook
              (defun akirak/org-follow-link-reveal ()
                (let ((buffer (window-buffer (selected-window))))
                  ...
                  (when (and (eq 'org-mode (buffer-local-value 'major-mode buffer))
                             (fboundp 'org-dog-indirect-buffer))
...
#+begin_src emacs-lisp
  (setup (:package org-placeholder)
    (with-eval-after-load 'akirak-unknown
      (define-key akirak-unknown-text-map "o" #'org-placeholder-find-or-create-1)))
#+end_src
...
| [[org-ql-search:tags:library][library]]             |  10 |
