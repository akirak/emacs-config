...
#+begin_src emacs-lisp
  (setup window
    ...
    (advice-add 'org-src-switch-to-buffer :around #'akirak/ad-no-obey-display-actions)
    ...
    (let ((modes '(help-mode
      ...
      (add-to-list 'display-buffer-alist
                   `((or ,@(mapcar (lambda (mode) (cons 'major-mode mode))
                                   modes))
                     akirak-window-reuse-mode-window-or-split-below
                     mode ,@modes)
                   t))
    (add-to-list 'display-buffer-alist
    ...
    (:with-mode org-mode
      ...
      (:when-loaded
        ...
        (add-to-list 'org-file-apps
                     `("\\.sqlite3?\\'"
                       . ,(defun akirak/org-file-open-sqlite (file &rest _)
    ...
    (:status personal
      (:when-loaded
        (org-dog-context-set-alist 'project
          :callback
          ;; I usually override this with a more personalized one
          (defun akirak/org-dog-project-context-1 (project)
...
**** flymake-bean-check
:PROPERTIES:
:CREATED_TIME: [2023-05-21 Sun 18:11]
:END:
#+begin_quote
A Flymake backend for bean-check
[[https://github.com/akirak/flymake-bean-check]]
#+end_quote

#+begin_src emacs-lisp
  (setup (:package flymake-bean-check)
...
| [[org-ql-search:tags:packages][packages]]            | 208 |
...
| [[org-ql-search:tags:beancount][beancount]]           |   2 |
...
|--------------------+------|
| [[beancount][beancount]]          |      |
| [[flymake-bean-check][flymake-bean-check]] |      |
| [[copilot][copilot]]            |      |
| [[dune][dune]]               |      |
| [[flycheck][flycheck]]           |      |
